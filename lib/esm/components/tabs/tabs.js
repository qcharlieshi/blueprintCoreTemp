...
import { Menu, MenuItem, OverflowList, Popover } from "..";
...
var Tabs = /** @class */ (function (_super) {
    ...
    function Tabs(props) {
        ...
        _this.overflowRenderer = function (items) {
            var firstId = null;
            var overflowMenu = (React.createElement(Menu, null, items.map(function (item) {
                if (item.props.id) {
                    firstId = item.props.id;
                    var onClick = function (e) {
                        item.props.onClick(item.props.id, e);
                    };
                    return (React.createElement(MenuItem, { key: item.props.id, text: item.props.title, onClick: onClick, disabled: item.props.disabled, active: item.props.selected }));
                }
                else {
                    return item;
                }
            })));
            return (React.createElement(Popover, { content: overflowMenu },
                React.createElement(TabTitle, { id: firstId + "-overflow", title: "Overflow" })));
        };
    ...
    Tabs.prototype.render = function () {
        ...
        return (React.createElement("div", { className: classes },
            React.createElement("div", { className: tabListClasses, onKeyDown: this.handleKeyDown, onKeyPress: this.handleKeyPress, ref: this.refHandlers.tablist, role: "tablist" },
                tabIndicator,
                this.props.overflow ? (React.createElement(OverflowList, tslib_1.__assign({ items: tabTitles, visibleItemRenderer: this.visibleItemRenderer, overflowRenderer: this.overflowRenderer, ref: this.refHandlers.overflowList }, this.props.overflowListProps, { onOverflow: this.onOverflow }))) : (tabTitles)),
            tabPanels));
        var _b, _c;
    };
    ...
    Tabs.prototype.moveSelectionIndicator = function (onOverflow) {
        ...
        window.requestAnimationFrame(function () {
            ...
            if (selectedTabElement !== null) {
                ...
                var overflowListElement = collapseFrom === Boundary.END ? overflowChildren[numTabChildren - 2] : overflowChildren[0];
